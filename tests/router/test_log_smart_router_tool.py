...
class TestLogSmartRouterMCPTool(unittest.TestCase):
    ...
    def test_accepts_request_limits_and_preserves_sdk_errors(self):
        self.router.log_search_client.search_logs.return_value = {"error": "Log search failed: unavailable"}
        result = asyncio.run(self.router.manage_logs("search", {
        ...
        self.assertEqual(result["results"], {"error": "Log search failed: unavailable"})
        kwargs = self.router.log_search_client.search_logs.call_args.kwargs
        self.assertEqual(kwargs["retrieval_size"], 200)
        self.assertEqual(kwargs["offset"], 2000)
        self.assertEqual(len(kwargs["requested_tags"]), 10)
