...
def main():
    ...
    try:
        ...
        parser.add_argument(
            ...
            type=str,
            metavar='<categories>',
            help="Comma-separated list of tool categories to enable (--tools infra, app, events, automation, mobile_app, website, settings, slo, releases, maintenance, synthetic, logs). Also controls which prompts are enabled. If not provided, all tools and prompts are enabled. Use 'router' for smart routing across app and infra metrics."
        )
        parser.add_argument(
            "--list-tools",
            action="store_true",
        ...
        all_categories = {"app", "infra", "events", "automation", "website", "mobile_app", "settings", "slo", "releases", "maintenance", "synthetic", "logs"}
        ...
        if invalid:
            logger.error(f"Error: Unknown category/categories: {', '.join(invalid)}. Available categories: app, infra, events, automation, mobile_app, website, settings, slo, releases, maintenance, synthetic, logs")
            sys.exit(2)
